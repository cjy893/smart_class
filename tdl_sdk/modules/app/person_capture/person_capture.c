...
CVI_S32 _PersonCapture_QuickSetUp(cvitdl_handle_t tdl_handle, person_capture_t *person_cpt_info,
  ...
  if (strcmp(od_model_name, "mobiledetv2-person-vehicle") == 0) {
    ...
    person_cpt_info->od_model_index = CVI_TDL_SUPPORTED_MODEL_YOLOV8_DETECTION;
  } else {
  ...
  if (ret != CVI_TDL_SUCCESS) {
    printf("OpenModel failed for index %d, path %s, ret=0x%x\n",
           person_cpt_info->od_model_index, od_model_path, ret);
    return ret;
  }
